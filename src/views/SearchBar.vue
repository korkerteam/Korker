...
<template>
  <form class="searchbar" role="search" @submit.prevent="onSubmit">
    <label class="search-input" aria-hidden>
      ...
      <input type="search" name="q" placeholder="Szukaj korepetycji, przedmiotów, nauczycieli..." aria-label="Szukaj" />
    </label>
...
.submit-btn {
  background: linear-gradient(90deg, #4f75c7 0%, #6b8ef0 100%);
  color: #fff;
  border: none;
  padding: 10px 16px;
  border-radius: 999px;
...
.submit-btn:hover { transform: translateY(-2px); box-shadow: 0 14px 36px rgba(79,117,199,0.22); }
.submit-btn:active { transform: translateY(0); }

@media (max-width: 900px) {
  .searchbar { padding: 4px; }
  .submit-btn { padding: 8px 12px; }
}
</style>
